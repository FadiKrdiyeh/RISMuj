...
function drawMinutes(dayDate, hour, sh, myColor) {
    ...
    for (var i = 0; i < myminutes.length; i++) {
        ...
        minuteRows += "<td id='" + dayDate + "_" + sh + "_" + myminutes[i] + "'>" + "<input type='button' class='form-control img-circle' style='background:#808080' onclick='addAppp(" + '"' + xox + '"' + ")'/>" + "</td>";
    }
...
(
{
    type: "POST",
    dataType: "json",
    url: "/Appoinments/pyramidSearch/",
    data: { dates: myCells, myShorthours: myShorthours, myminutes: myminutes, clinic: $('#mmnmClinic').val() },
    success: function (d) {
        for (var i = 0; i < myCells.length; i++) {
            ...
            if (d.data[i].appId != "-1") {
                ...
                $('#' + myCells[i]).empty();
                //myTd.empty();
                var newBtn = "<input type='button' class='form-control img-circle' style='background:red;border-radius: 19px;'  ondblclick='showApp(" + d.data[i].appId + ")'/>";
    ...
    $.ajax
        (
        {
            ...
            success: function (d) {
                ...
                $('#mnmDatOld').append(d.data[0].appDate);
                $('#mnmDatOld').append("<br/>");
                $('#mnmDatOld').append("العيادة");
                $('#mnmDatOld').append("<br/>");
                $('#mnmDatOld').append(d.data[0].clinicName);
    ...
    $.ajax
        (
        {
            ...
            success: function (d) {
                ...
                $('#mnmFastDatOld').append(d.data[0].appDate);
            }
        });
}
